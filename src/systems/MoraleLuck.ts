...
export class MoraleLuckSystem {
  ...
  checkMorale(unit: BattleUnit, hero: Hero | null): 'positive' | 'negative' | 'none' {
    ...
    if (unit.creatureId === 'angel' || unit.creatureId === 'archangel') {
      morale += 1;
    } else if (unit.creatureId === 'skeleton' || unit.creatureId === 'zombie' || unit.creatureId === 'vampire') {
      // Нежить не подвержена морали
      return 'none';
    }

    // Шанс срабатывания: |morale| × 8%
    const chance = Math.abs(morale) * 0.08;
  ...
  checkLuck(attacker: BattleUnit, hero: Hero | null): 'critical' | 'fumble' | 'none' {
    ...
    // У ангелов удача +1
    if (attacker.creatureId === 'angel' || attacker.creatureId === 'archangel') {
      luck += 1;
    }

    // Шанс срабатывания: |luck| × 8%
    const chance = Math.abs(luck) * 0.08;
